...
export const PizzaCarousel = ({ pizzas, onAddItem, onEditPizza, onDeletePizza }: PizzaCarouselProps) => {
  ...
  /** Keeps the position valid when the list shrinks under the stored index. */
  const safeStartIndex = pizzas.length === 0 ? 0 : startIndex % pizzas.length;
  ...
  const handleNext = () => {
    ...
    setStartIndex((safeStartIndex - 1 + pizzas.length) % pizzas.length);
  };

  const handlePrevious = () => {
    if (pizzas.length === 0) return;
    setSlideDirection('right');
    setStartIndex((safeStartIndex + 1) % pizzas.length);
  };
        ...
        {Array.from({ length: totalPositions }, (_, index) => (
          <Box
            ...
            sx={{
              width: '100%',
              height: '100%',
              display: safeStartIndex === index ? 'flex' : 'none',
              alignItems: 'stretch',
              justifyContent: 'center',
              py: 2,
            }}
          >
            <Slide direction={slideDirection} in={safeStartIndex === index} mountOnEnter unmountOnExit>
              <Stack
                spacing={2}
                direction="row"
                sx={{
